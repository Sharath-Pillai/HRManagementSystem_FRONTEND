  ...
  const columns = [
    {
      ...
      accessor: "date",
      sortable: true,
      render: (r) => format(new Date(r.date), "dd MMM yyyy, EEE"),
    },
    ...(!isEmployee
      ? [
          {
    ...
    {
      header: "Check In",
      render: (r) => (r.checkIn ? format(new Date(r.checkIn), "hh:mm a") : "—"),
    },
    {
      header: "Check Out",
      render: (r) =>
                  ...
                  <div className="mt-3 flex flex-wrap gap-4 text-sm">
                    {today.checkIn && (
                      <span className="text-slate-300">
                        🟢 In: {format(new Date(today.checkIn), "hh:mm a")}
                      </span>
                    )}
                    {today.checkOut && (
                      <span className="text-slate-300">
                        🔴 Out: {format(new Date(today.checkOut), "hh:mm a")}
                      </span>
                    )}
                    {today.totalHours > 0 && (
                      <span className="text-emerald-400 font-medium">
                        ⏱ {today.totalHours}h worked
                      </span>
                    )}
                  </div>
