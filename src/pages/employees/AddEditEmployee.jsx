  ...
  useEffect(() => {
    if (form.department) {
      setFilteredDesig(
        designations.filter(
          (d) =>
            d.department?._id === form.department ||
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Object.entries(form).forEach(([k, v]) => {
        ...
        else if (k === "address" || k === "emergencyContact")
          fd.append(k, JSON.stringify(v));
        else if (v !== "") fd.append(k, v);
      });
      let response;
      if (isEdit) {
        response = await api.put(`/employees/${id}`, fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      } else {
        response = await api.post("/employees", fd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
      }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <FormGroup label="Relation">
              <input
                ...
                onChange={(e) =>
                  setNested("emergencyContact", "relation", e.target.value)
                }
                className="input"
              />
            </FormGroup>
